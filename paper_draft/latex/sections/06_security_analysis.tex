...
\subsection{Nonlinear-island structural leakage (cross-Gram and value multiset)}
\label{sec:security:nonlinear}
The operator-compatible nonlinear islands (paired channel permutations for GELU/ReLU/SwiGLU) are exact but not value-hiding: inside an island the GPU observes the pre-activation up to a per-call channel permutation, so it can compute the token$\times$token activation--gradient Gram $\Zt\GZt^{\top} = \Z\GZ^{\top}$ and the per-token value multiset (the sorted rows of $\Zt$) exactly, and, during training, the per-token gradient norm $\lVert\GZt\rVert = \lVert\GZ\rVert$. We evaluate whether this structural leakage recovers user data with a five-attacker panel (token reconstruction, private label/target, membership, LoRA adapter/$\Delta\W$ extraction, future-token), separately under the \emph{private-base} main setting and a \emph{public-base ablation}.

\paragraph{Main setting (private base weights).}
Because the attacker does not hold plaintext $\W$ (\Cref{sec:threat}), it cannot map an observed value multiset to a vocabulary item, solve the per-call channel permutation, or fit a surrogate adapter. In our tests \textbf{token reconstruction, target/label recovery, future-token prediction, and adapter/$\Delta\W$ extraction all fail} (token top-1 and future-token top-1 fall to the frequency baseline; adapter $\Delta\W$ cosine $\approx 0$). The one residual channel is \textbf{membership inference at AUC $\approx 0.61$}, which rides the model-agnostic gradient-norm signal and does not depend on knowing $\W$; we report this as \texttt{medium} residual leakage. The pure Gram matrix alone---without the co-leaked value multiset---carries no vocabulary identity (token top-1 at baseline). We therefore classify the nonlinear cross-Gram / value-multiset as \emph{auxiliary structural leakage}: it is exact and non-zero, and it is \emph{not} eliminated, but under the evaluated attackers it did not recover user tokens, targets, or a functional adapter. Risk: \texttt{needs\_more\_evaluation} for reconstruction channels, \texttt{medium} for membership.

\paragraph{Public-base ablation (negative setting).}
When the base weights are instead public---the assumption used by much prior obfuscation work, included here only as an ablation---the same leak becomes \texttt{high}: value-multiset candidate matching against $\mathrm{sort}(\mathrm{emb}\,\W)$ gives exact token reconstruction (top-1 $\approx 1.0$), which then enables target recovery (AUC $\approx 0.82$), future-token recovery, and, via the recovered global channel permutation, full $\Delta\W$ adapter extraction (cosine $\approx 1.0$). This ablation is what motivates the private-base threat model: the danger is the co-leaked pre-activation value multiset in combination with plaintext weights, not the Gram matrix in isolation.
...
\input{tables/security_proxy_summary.tex}
