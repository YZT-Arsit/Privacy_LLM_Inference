...
13. **Base-weight confidentiality is a mask-secrecy proxy claim, not a formal guarantee.** The base-model weights are proprietary/private and are not revealed to the untrusted GPU/host: only the masked form `W_tilde = N_in^{-1} W N_out` crosses the boundary, and plaintext `W` never leaves the trusted side. Under the private-base setting the protected assets are the private base weights, user fine-tuning data, LoRA adapter, gradients, KV cache, and hidden states. Weight confidentiality reduces to the secrecy of the fresh per-call masks and is proxy-evaluated (Security Analysis), not cryptographic; a dedicated weight-recovery attacker under a *stronger* model than the ones we run is out of scope. This is also the setting under which the public-weight cross-Gram / value-multiset recovery attacks (reported as an ablation) do *not* succeed, precisely because the attacker no longer holds plaintext `W`.
...
15. **Adapter merging into `W` is intentionally not supported.** The Stage 7.0 contract is that the LoRA adapter is *never* merged into the base weight; doing so would publish a derived weight that no longer admits the masked forward identity of Theorem 7.
