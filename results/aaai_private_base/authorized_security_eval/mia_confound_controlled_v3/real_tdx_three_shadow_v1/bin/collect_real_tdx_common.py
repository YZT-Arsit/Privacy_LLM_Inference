...
def trace_forward(model: MaskedQwen, ids: torch.Tensor, names: list[str], seed: int,
    ...
    # common-semantics schema v2 intentionally excludes hidden.final.last_l2:
    # the simulated post-gamma point is not GPU-visible in the transformed path.
...
def main() -> None:
    ...
    names = [x["feature_name"] for x in schema["features"]]
    if len(names) != 610 or len(set(names)) != 610 or "hidden.final.last_l2" in names:
        raise RuntimeError("common schema is not 610 unique features with hidden.final excluded")
    queries = [json.loads(x) for x in args.queries.read_text().splitlines() if x.strip()]
    if len(queries) != 1000:
        raise RuntimeError("query count is not 1000")
    ids = [x["sample_id"] for x in queries]
    id_hash = hashlib.sha256(("\n".join(ids) + "\n").encode()).hexdigest()
    if id_hash != "d077509179a93d8873bb5254993040ad336b08f48ed0704a5c9de0d1e59b845c":
    ...
    manifest = {
        "schema": "real_tdx_three_shadow_common_v1", "status": "PASS",
        "collection_mode": "REAL_TDX_BACKED_VIEW",
        "collection_scale": "REAL_TDX_THREE_SHADOW_FULL",
        "scope": "frozen 1000-sample candidate pool",
