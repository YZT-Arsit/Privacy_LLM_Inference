...
(`results/aaai_ts_optimized/`) with the plaintext responses + MT-Bench judge from
`results/aaai_fp32_final/`.
...
- Per-token, the trusted boundary adds only ~0.8 ms; the gap is the untrusted
  ...
  `results/generation_backend_eval/`).
...
Sources: `results/aaai_ts_optimized/` (OURS, this run),
`results/aaai_fp32_final/` (plaintext responses + MT-Bench Claude-Opus judge),
`results/aaai/humaneval/` (HumanEval plaintext).
