...
Planned extensions after Stage 5.6 extension:

- Stage 7.0 (deferred) — LoRA private-training path under the same obfuscation envelope: mask scheduling under autograd, fresh-mask budget per step, gradient-side trace surface. Stage 5.6 extension closed the inter-block residual gap and the decode-step latency leakage (proxy-only), so Stage 7.0 inherits a clean inference baseline. Open questions to address in 7.0: how does `N_inter` interact with autograd through the SwiGLU island; can the fresh-mask budget be amortised across micro-batches; do gradient activations leak beyond what inference activations leak.
