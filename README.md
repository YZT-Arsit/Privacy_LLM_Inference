...
Planned extensions after Stage 6.4c:

- Stage 5.5b — Real-token-prompted real-activation attacker. Stage 5.5 collected traces from random hidden-state block input; Stage 5.5b uses the Stage 6.4c model-level wrapper with `--attempt-real-model-load` + tokenizer / embedding path to feed real prompts through real Qwen2.5-0.5B / TinyLlama-1.1B and re-run the Stage 5.4 adaptive attackers on those activations. Honest token distributions are the main remaining gap in the Stage 5.5 limitations list.
- Stage 5.6 (research) — Stronger attacker variants: black-box query against deployed models, side-channel-aware threat models, ML-based permutation recovery using cross-attention information. Only Stage 5.6 results can move `security_profile` from `"proxy-evaluated, not formal"` to a stronger label.
