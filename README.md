...
Planned extensions after Stage 7.0:

- Stage 7.1 (next) — Masked backward / gradient-side obfuscation. Stage 7.0 keeps backward + optimizer trusted; Stage 7.1 sends masked `grad_Y_tilde` to the GPU, recovers `grad_A_tilde` / `grad_B_tilde` on the trusted side, and only the trusted side applies the update. Open questions: how does `N_in` / `N_out` / `U` re-randomisation interact with autograd through the SwiGLU island; can `grad_Y` be masked end-to-end without inflating online matmuls; do gradient activations leak beyond what inference activations leak under the Stage 7.0 proxy.
- Stage 7.2 (deferred) — Rank padding to hide the LoRA rank `r` from the shape of `A_tilde` / `B_tilde`; this also covers cross-call rank-signature linkability.
- Stage 7.3 (deferred) — Multi-layer LoRA in a tiny transformer block end-to-end + calibrated LoRA workload + LoRA timing-side proxy.
