...
Planned extensions after Stage 5.3e:

- Stage 6.4b — Real Qwen / TinyLlama small-model loading and block-level wrapper integration behind the same `nonlinear_mode` feature flag (default `trusted`), now able to opt in to `mitigation_bundle="fresh_perm_plus_sandwich_plus_pad"` end-to-end.
- Stage 5.3d (deferred) — Full BERT and T5 obfuscated wrappers (not just probes). Stage 5.3e provides the per-call bundle wiring; Stage 5.3d wraps it into BertModel / T5Model `forward` + KV cache + LM head. Only after this can `ours_compatible_nonlinear_islands.implemented` flip to `True`, `wall_time_source` to `measured`, and `full_runtime_integrated` to `True`.
