"""Stage 7.5 — paper claims audit.

Classifies every paper-side claim into one of three buckets:

* ``supported`` — directly supported by a correctness or wall-time
  artifact (e.g. ``masked execution correctness holds for tested
  GPT-2 / modern-decoder wrappers``).
* ``proxy_supported`` — supported only by a proxy attacker / cost-model
  experiment, never a formal theorem. The paper must word these as
  "proxy-evaluated" findings, not "secure" guarantees.
* ``unsupported`` — claims the artifacts CANNOT support. These MUST NOT
  appear in the paper. Examples: formal / cryptographic / semantic
  security, real TEE wall-time, hardware side-channel resistance,
  ``padded_rank`` is hidden, optimizer fully outsourced.

For each claim the audit emits ``status`` / ``evidence_artifacts`` /
``paper_safe_wording`` / ``unsafe_wording_to_avoid`` / ``notes`` so the
paper writer never accidentally overclaims.
"""
...
_SUPPORTED_CLAIMS: tuple[dict[str, Any], ...] = (
    {
        "claim": "Masked execution preserves output equality vs the plain reference for the tested GPT-2 model-level wrapper (full forward + prefill + decode_step + greedy generation).",
        "evidence_artifacts": [
    ...
    {
        "claim": "Masked execution preserves output equality vs the plain reference for the tested modern-decoder model-level wrapper (RMSNorm + SwiGLU + RoPE + GQA, prefill + decode_step + greedy generation).",
        "evidence_artifacts": [
    ...
    {
        "claim": "Compatible nonlinear islands preserve output correctness across the tested architectures (decoder-only / encoder-only / encoder-decoder, plus modern decoder).",
        "evidence_artifacts": [
    ...
    {
        "claim": "KV cache append invariant holds for the tested decoder-only generation paths.",
        "evidence_artifacts": [
    ...
    {
        "claim": "LoRA masked forward, masked backward, and rank-padded forward/backward reproduce the plain rank-r reference to float64 precision in synthetic single-linear tests.",
        "evidence_artifacts": [
    ...
    {
        "claim": "Multi-layer rank-padded LoRA training reproduces the plain reference loss / gradient / SGD / AdamW update across every per-module slice in synthetic multi-layer tests.",
        "evidence_artifacts": [
    ...
    {
        "claim": "All Stage 7.4 stronger dummy distributions preserve A_pad B_pad = A_real B_real exactly (cancellation strategies) or with a tracked trusted-side correction (noise_injected_cancellation_dummy), and all five preserve forward / backward / SGD / AdamW update correctness to float64 precision.",
        "evidence_artifacts": [
    ...
    {
        "claim": "Constant-time training mode (proxy_equalized) reduces the cost-model timing classifier accuracy to near random chance in our tests.",
        "evidence_artifacts": [
...
_PROXY_SUPPORTED_CLAIMS: tuple[dict[str, Any], ...] = (
    {
        "claim": "Activation-recovery / linkability adaptive proxy attackers are bounded under the full mitigation bundle (fresh permutation + dense sandwich + boundary pad) in the tested configurations.",
        "evidence_artifacts": [
    ...
    {
        "claim": "Cost-model timing side-channel proxy is bounded under proxy_equalized constant-time decode / training modes in our tests.",
        "evidence_artifacts": [
            "outputs/stronger_attackers.json",
            "outputs/lora_training_timing_proxy.json",
        ],
    ...
    {
        "claim": "LoRA adapter extraction / gradient leakage proxy attackers are bounded under fresh masks + pad in the tested configurations.",
        "evidence_artifacts": [
    ...
    {
        "claim": "Rank padding hides true_rank from the GPU-visible tensor shape, and stronger dummy distributions reduce the proxy spectral-cliff inference accuracy of true_rank.",
        "evidence_artifacts": [
    ...
    {
        "claim": "Cross-layer adapter linkage is low under fresh masks + paired cancellation across the tested multi-layer configuration.",
        "evidence_artifacts": [
...
_UNSUPPORTED_CLAIMS: tuple[dict[str, Any], ...] = (
    {
        "claim": "Formal / cryptographic / semantic security of the masked path.",
        "evidence_artifacts": [],
    ...
    {
        "claim": "Real TEE wall-time / hardware-isolation evaluation.",
        "evidence_artifacts": [],
    ...
    {
        "claim": "Hardware side-channel security (cache / power / EM).",
        "evidence_artifacts": [],
    ...
    {
        "claim": "Full Qwen / TinyLlama / LLaMA LoRA fine-tuning.",
        "evidence_artifacts": [],
    ...
    {
        "claim": "PEFT / DeepSpeed / vLLM / FlashAttention compatibility.",
        "evidence_artifacts": [],
    ...
    {
        "claim": "padded_rank is hidden from the GPU.",
        "evidence_artifacts": [],
    ...
    {
        "claim": "Loss / optimizer is fully outsourced to the GPU.",
        "evidence_artifacts": [],
    ...
    {
        "claim": "Protection against a compromised TEE.",
        "evidence_artifacts": [],
...
def _md(rows: list[dict[str, Any]]) -> str:
    lines: list[str] = []
    lines.append("# Paper Claims Audit\n")
    lines.append(
        "Each claim is classified as `supported` (direct artifact"
        " evidence), `proxy_supported` (proxy attacker / cost-model"
        " evidence only), or `unsupported` (do NOT write into the"
        " paper)."
    )
    lines.append("")
    ...
    for status in ("supported", "proxy_supported", "unsupported"):
        lines.append(f"## {status}\n")
        for entry in by_status.get(status, []):
            lines.append(f"### {entry['claim']}\n")
            lines.append(f"- **status:** `{entry['status']}`")
            if entry.get("evidence_artifacts"):
                lines.append("- **evidence_artifacts:**")
                for art in entry["evidence_artifacts"]:
                    lines.append(f"  - `{art}`")
            else:
                lines.append("- **evidence_artifacts:** _(none — this is unsupported)_")
            lines.append(
                f"- **paper_safe_wording:** {entry['paper_safe_wording']}"
            )
            lines.append(
                f"- **unsafe_wording_to_avoid:** {entry['unsafe_wording_to_avoid']}"
            )
...
def _latex(rows: list[dict[str, Any]]) -> str:
    ...
    lines: list[str] = [
        ...
        r"\caption{Paper claims audit: supported, proxy-supported, unsupported.}",
        r"\label{tab:paper_claims_audit}",
        r"\begin{tabular}{p{1.6cm}p{6cm}p{6cm}}",
        r"\toprule",
        r"\textbf{Status} & \textbf{Claim} & \textbf{Paper-safe wording} \\",
        r"\midrule",
    ]
    for r in rows:
        lines.append(
...
def run_paper_claims_audit(
    config: PaperClaimsAuditConfig,
) -> dict[str, Any]:
    rows: list[dict[str, Any]] = []
    for entry in _SUPPORTED_CLAIMS:
        rows.append({**entry, "status": "supported"})
    for entry in _PROXY_SUPPORTED_CLAIMS:
        rows.append({**entry, "status": "proxy_supported"})
    for entry in _UNSUPPORTED_CLAIMS:
        rows.append({**entry, "status": "unsupported"})

    # Validate that the supported claims reference real artifacts.
    outputs_dir = Path(config.outputs_dir)
    for r in rows:
        if r["status"] == "unsupported":
            continue
        missing = [
            a for a in r.get("evidence_artifacts", [])
            if not (outputs_dir.parent / a).exists()
            and not (outputs_dir / Path(a).name).exists()
        ]
        if missing:
            r.setdefault("notes", "")
            r["notes"] = (
                (r.get("notes") or "")
                + f" [missing artifacts: {missing!r}]"
            )
    ...
    (paper_results_dir / "json" / "paper_claims_audit.json").write_text(
        json.dumps({"claims": rows, "config": asdict(config)},
                   indent=2, default=str),
        encoding="utf-8",
    )
    (paper_results_dir / "markdown" / "paper_claims_audit.md").write_text(
        _md(rows), encoding="utf-8",
    )
    (paper_results_dir / "latex" / "paper_claims_audit.tex").write_text(
        _latex(rows), encoding="utf-8",
    )
    ...
    return {
        ...
        "limitations": [
            "Audit is curated; the registry must be kept in sync with new stages.",
            "Reports publish summary metadata only; no raw tensors / masks / adapters / gradients.",
            "Unsupported claims MUST NOT appear in the paper as guarantees.",
        ],
    }
